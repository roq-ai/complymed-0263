...
import { getAuditById } from 'apiSdk/audits';
...
function AuditViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<AuditInterface>(
    () => (id ? `/audits/${id}` : null),
    () =>
      getAuditById(id, {
        relations: [],
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                <FormListItem
                  label="Audit Date"
                  text={data?.audit_date ? format(parseISO(data?.audit_date as unknown as string), 'dd-MM-yyyy') : ''}
                />
