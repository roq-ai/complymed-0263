...
import { getAudits } from 'apiSdk/audits';
...
function DashboardPage() {
  ...
  const auditStat = useSWR<PaginatedInterface<AuditInterface>>(
    () => `/audits`,
    () => getAudits({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Audits',
      result: auditStat?.data?.totalCount,
      isLoading: auditStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
